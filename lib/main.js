    ...
    */
    ...
    directLeaseOffer: function(options) {
      ...
      function finishDirectLeaseOffer(isFuture) {
        var offer = Offers.insert(o)
        var msg = ['You have a new request for leasing ', options.assets.map(function(a) { return a.name }).join(', '), ' assets. Please check your active negotiations under DASHBOARD to access this item.']


        if (o.receipt) {
          msg = msg.concat([' An escrow payment of $', (options.receipt.amount/100).toFixed(2), ' is collected and available for this item upon approval.'].join(''))
        }
