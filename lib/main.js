...
var StripeServerKey = 'sk_test_NV3x8Jk24lPX5ToEccddXGLq';
...
Meteor.methods({
  ...
  upgradeProfile: function(options) {
    ...
    if (Meteor.isClient) {
      ...
      var ipAddr = this.connection.clientAddress;
    ...
    if (Meteor.isServer) {
      ...
      stripe.charges.create({
        ...
        if (charge) {
          ...
          options.message = myName()+' purchased '+options.gift.name+' for $'+options.amount.toFixed(2);
          options.uid = Meteor.user()._id;
          options.slug = project.slug;
          options.created = new Date();
          delete options['token'];
  ...
  acceptUserToProject: function(options) {
    ...
    if (Meteor.isServer) {
      console.log(options)
      console.log('iu219012308938100000011123')
      var key=options.ctx==='crew'?'crewApplicants':'roleApplicants';
      var project = Projects.findOne({slug: options.slug});
      delete options['slug'];
      ...
      if (options.type==='hired') {
        ...
        createNotification({
          ...
          message: options.user.name+' donated $'+options.pay+' for your acceptance.',
          title: project.title,
          slug: project.slug,
          purpose: 'donation'
        });
      ...
      function finalUpdateHandler(idx) {
        ...
        var updateObj = { usersApproved: usersApproved, equityAllocated: equityAllocated, funded: funded };
  ...
  transferFunds: function(options, cb) {
    ...
    if (Meteor.isServer) {
      ...
      if (availableTransfer*1.1<1) return 'your funds have already been transferred';
      ...
      stripe.transfers.create({
        ...
        transfer_group: project.slug,
      }).then(Meteor.bindEnvironment(function(err, transfer) {
        // asynchronously called
        if (err) return future.return(err.message);
        createReceipt({
          title: project.title,
          slug: project.slug,
          amount: availableTransfer,
          purpose: 'transfer',
          receipt: transfer,
        });
        future.return('your transfer was made, and will be processed during the next business week');
      }));
      return future.wait();
    };
  }
