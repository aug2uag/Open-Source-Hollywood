...
var StripeServerKey = 'sk_live_Wc9rRa8LbtyVkV6XVm7vAAAu';
...
function getMiniURL(url) {
  var requestParams = {
    uri:    'https://www.googleapis.com/urlshortener/v1/url?key=AIzaSyCdwU-o-DMiSdBivkw-QHzYAfzqyQxVe98',
    method: 'POST',
    json: {'longUrl': url}
  };
  var future = new (Npm.require('fibers/future'))();
  request( requestParams, Meteor.bindEnvironment(function ( err, body, response ) {
...
Meteor.methods({
  setNotificationPreferences: function(options) {
    ...
    if (email) {
      ...
      var guid1 = guid();
      var guid2 = guid();
      if (email_preferences.email===email&&email_preferences.verification===true) {
        /** do nothing, email already verified */
      } else {
        /** set guids to user */
        ...
        var params = {
          ...
          Message: {
           Body: {
            Html: {
             Charset: "UTF-8", 
             Data: "Your email was provided as a source for notifications on Open Source Hollywood. To verify, please visit the following link: <a class=\"ulink\" href=\"" + verificationURL + "\" target=\"_blank\">Click Here to Verify Email</a>."
            }, 
            Text: {
             Charset: "UTF-8", 
             Data: "Your email was provided as a source for notifications on Open Source Hollywood. To verify, please visit the following URL: " + verificationURL
            }
           }, 
           Subject: {
            Charset: "UTF-8", 
            Data: "Verify Email for Notifications from Open Source Hollywood"
           }
          }, 
          ReplyToAddresses: [
    ...
    if (phone) {
      ...
      if (phone.length!==10) {
      ...
      } else if (phone_preferences.phone===phone&&phone_preferences.verification===true) {
        ...
        var __opts = {
          to: phone,
          from: '+14157877947 ',
          body: verificationCode + ' is your verification code for Open Source Hollywood.',
        }
        var options = {
          method: 'post',
          body: __opts,
  ...
  addProject: function (options) {
    ...
    if (Meteor.isServer) {
      ...
      // getMiniURL
      var url = 'https://app.opensourcehollywood.org/projects/'+options.slug+'/'+options.ownerId;
      var miniURL = getMiniURL(url);
      if (miniURL.id) {
        options.urlLink = miniURL.id;
      } else {
        options.urlLink = url;
      }
  ...
  donateToProject: function(options, cb) {
    ...
    if (Meteor.isServer) {
      options.user = Meteor.user() ? {
        ...
        id: 'anon',
        name: 'patron',
        avatar: 'https://upload.wikimedia.org/wikipedia/commons/thumb/2/29/Gold_Star.svg/2000px-Gold_Star.svg.png'
      }
      ...
      stripe.charges.create({
        ...
        if (charge) {
          ...
          createNotification({
            user:project.ownerId,
            message:myName() + ' donated $' + (options.amount).toFixed(2),
            title: project.title,
            slug: project.slug,
            purpose: 'donation'
          });
  ...
  addProjectMessage: function(options) {
    if (Meteor.isServer) {
      ...
      ProjectMessages.insert({
        ...
        ownerName: Meteor.user().services.auth0.name,
        ownerAvatar:Meteor.user().services.auth0.picture,
        createdAt: moment().format("MMMM D, YYYY"),
        createTimeActual: moment().format()
      });
    } 
