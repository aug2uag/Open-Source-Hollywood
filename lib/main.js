...
Meteor.methods({
    ...
    if (Meteor.isServer) {
      ...
      function finalUpdateHandler(idx) {
        ...
        if (email_preferences.email&&email_preferences.verification===true) {
          ...
          sendEmailNotification(email_preferences.email, html, textMessage, 'Application Decision from O . S . H .');
        }
  ...
  buyShares: function(options) {
    ...
    while (options.shares > 0) {
      if (totalShares > options.shares) {
        sharesPurchased+=options.shares;
    ...
    if (Meteor.isServer) {
      ...
      stripe.charges.create({
        ...
        if (charge) {
          ...
          // update project
          Projects.update({_id: project._id}, { $set: { shares: projectShares, availableShares: totalShares }});
          // create receipt
          createReceipt({
            title: project.title,
            slug: project.slug,
          ...
          future.return('You purchased ' + sharesPurchased + ' shares at $' + project.mpps + ' per share. ' + new Date.toLocaleDateString() + ' O . S . H .');
        } else {
