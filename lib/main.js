  ...
  rejectUserFromProject: function(offers) {
    ...
    if (Meteor.isServer) {
      ...
      for (var i = 0; i < offers.length; i++) {
        var o = offers[i]
        if (o.receipt) {
          stripeTransactions.push(o.receipt.id)
      ...
      function declinedUserRefund(tx, cb) {
      ...
      function declinedUserTransactionsProcess(tx) {
        if (!tx) {
          return finalRejectApplicantHandler();
        } else {
          return declinedUserRefund(tx, function(err) {
