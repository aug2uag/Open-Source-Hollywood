...
Meteor.methods({
  ...
  rejectUserFromProject: function(offers) {
    ...
    var message = '';
    if (Meteor.isServer) {
      var project = Projects.findOne({slug: offers[0].slug});
      ...
      function innerUpdateProjectDeclinedApplicants(o) {
        var key=o.ctx==='crew'?'crewApplicants':'roleApplicants';
        var applicants = project[key];
        for (var i = 0; i < applicants.length; i++) {
          var el = applicants[i];
          if (o.ctx===el.ctx&&o.type===el.type&&o.pay===el.pay&&o.equity===el.equity&&o.appliedFor===el.appliedFor&&o.created.toString()===el.created.toString()) {
            if (o.receipt&&el.receipt) {
              if (o.receipt.id===el.receipt.id) {
                if (idx===offers.length) return finalUpdateHandler(i, o);
              };
            };
          };
        };
      ...
      function finalUpdateHandler(idx, o) {
        /** TODO: automatically reject other users applying for the same role?? */
        // add user to project
        applicants.splice(i,1);
        var updateObj = { };
      ...
      offers.forEach(function(o, idx) {
        if (o.declined&&o.receipt) {
          receipts.push(o.receipt);
          message += 'Your offer for '+o.appliedFor+' was declined and you were refunded $'+o.pay+'. ';
          /** update project applicants */
          return innerUpdateProjectDeclinedApplicants(o);
        };
      });
    };
  },
  ...
  pokeApplicant: function(options) {
    ...
    //   if (a.uid === options.uid) a.poke = true;
    ...
    // });
    Projects.update({slug: options.slug}, {$set: { 
      roleApplicants: roleApplicants, crewApplicants: crewApplicants }});
    ...
    var miniURL = getMiniURL(fullURL);
    var msg = (options.audition&&options.audition!=='N/A') ? 'You\'ve been selected to submit audition information, please upload your audition' : 'You have been elected to negotiate your offer';
    ProjectMessages.insert({
      project: options.project,
      slug: options.slug,
      title: project.title,
  ...
  authorFinalizeAgreement: function(options) {
    ...
    if (Meteor.isServer&&options.project&&options.project._id) {
      ...
      if (!offersAgg.length) return false;
      ...
      try {
        priceToPay = offersAgg.filter(function(o) {
          if (o.type==='hired') return o.pay
        }).reduce(function(a, b) {
          return a||0 + b||0
        })
      } catch(e) {}
      ...
      try {
        amountReceivable = offersAgg.filter(function(o) {
          if (o.type!=='hired') return o.amount
        }).reduce(function(a, b) {
          return a||0 + b||0
        })
      } catch(e) {}
      ...
      try {
        equityOut = offersAgg.reduce(function(a, b) {
          return a&&a.equity||0 + b&&b.equity||0
        })
      } catch(e) {}
