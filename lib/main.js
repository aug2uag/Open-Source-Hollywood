...
Meteor.methods({
    ...
    addProject: function (options, cb) {
        ...
        if (previousProjectExists) {
          ...
          if(future)future.throw(new Meteor.Error("Error: duplicate title", "This campaign contains a title identical to a previous project of yours. Please update the campaign to a new title so it is unique."))
          else return "This campaign contains identical title and genre to an existing and active project of yours. Please update the campaign title to a new one so it is unique."
        }
        if (/** country ==US, postal_code is 5 digits */!options.country||options.country==='US') {
          if (!options.zip.match(/\d{5}/)) {
            if(future)future.throw(new Meteor.Error("invalid zip", "Invalid location, please enter a valid ZIP or postal code."));
            else return "Invalid location, please enter a valid ZIP or postal code."
          };
        };
        var requestParams = {
          uri:    ['https://maps.googleapis.com/maps/api/geocode/json?address=',options.zip,',',options.country||'US','&key=AIzaSyDDUJ4ZQNWvF4x2C7AbnPR8-PZEoQGCnLs'].join(''),
        ...
        request( requestParams, Meteor.bindEnvironment(function ( err, response, geoOptions ) {
          if (!geoOptions) {
            if(future)future.throw(new Meteor.Error("campaign create", "Invalid location, please enter a valid ZIP or postal code."));
            else return "Invalid location, please enter a valid ZIP or postal code."
          };
