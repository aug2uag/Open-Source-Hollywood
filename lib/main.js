...
Meteor.methods({
  ...
  donateToProject: function(options, cb) {
    ...
    if (Meteor.isServer) {
      ...
      stripe.charges.create({
        ...
        // if (err||!charge) 
          return future.throw('payment failed');
        if (charge) {
  ...
  lendResource: function(options) {
    ...
    if (Meteor.isServer) {
      ...
      var mappedStings = offeredResources.map(function(o){return JSON.stringify(o)});
      // prevent duplicates
      if (mappedStings.indexOf(JSON.stringify(options))===-1) offeredResources.push(options);
  ...
  applyToProject: function(options, cb) {
    ...
    if (Meteor.isServer) {
      ...
      var mappedStings = mappedValues.map(function(o){return JSON.stringify(o)});
      // prevent duplicates
      if (mappedStings.indexOf(JSON.stringify(options))===-1) mappedValues.push(options);
      else return cb('this project already has your ' + key + ' application, please negotiate in messages');
      Projects.update({_id: project._id}, { $set: { key: mappedValues }});
      Notifications.insert({
        user: project.ownerId,
        from: Meteor.user()._id,
        fromAvatar: Meteor.user().avatar,
      ...
      if (true) {
        console.log(new Array(100).join('io '))
        console.log(options)
      };
    }
  },
