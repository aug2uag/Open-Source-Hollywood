    ...
    */
    ...
    leaseRequest: function(options) {
      ...
      function updateOfferLeaseReq() {
        ...
        var offerorMsg = msg3 ? msg2.concat(msg3).join(' ') : msg2.join('')
        var offereeMsg = msg3 ? msg1.concat(msg3).join(' ') : msg1.join('')
        createNotification({
            user: offer.offeror,
            message: offerorMsg,
      ...
      if (options.token) {
        ...
        stripe.charges.create({
          ...
          currency: "usd",
          source: options.token.id,
          transfer_group: options.offer.offeree
        }, Meteor.bindEnvironment(function(err, charge) {
          if (err) return future.return(err.message);
          if (charge) {
      ...
      console.log('foodaddy')
      console.log(options)
    },
    revokeLeaseRequest: function(options) {
      check(options, Object)
      if (Meteor.isClient) return
      console.log('foodaddy   foodaddy   foodaddy   foodaddy   foodaddy   foodaddy   ')
      console.log(options)
    ...
    rejectLeaseRequest: function() {
      // refund if receipt
      // update object to rejected
      // notify rejection and/or refund 

    },
    approveLeaseRequest: function() {
