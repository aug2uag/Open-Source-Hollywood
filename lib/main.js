...
Meteor.methods({
        ...
        if (Meteor.isServer) {
          ...
          request( requestParams, Meteor.bindEnvironment(function ( err, response, geoOptions ) {
            ...
            createNotification({
              ...
              message: 'Campaign created! You created a new campaign, and it is now displayed live and eligible for viewing and user actions. Be sure to click on the social media links and spread the word to your network of friends and family for contributions and shares. If we can help you do not hesitate to reach out to us on social media or by email to "hello@opensourcehollywood.org" and one of our representatives will be in touch with you soon.',
              title: options.title,
              slug: options.slug,
    ...
    lendResource: function(options) {
      ...
      if (Meteor.isServer) {
        ...
        if (Meteor.user().notification_preferences&&Meteor.user().notification_preferences.email&&Meteor.user().notification_preferences.email.verification) {
          options.user.email = Meteor.user().notification_preferences.email.em
        };
        ...
        if (Meteor.isServer) {
          ...
          stripe.charges.create({
            ...
            if (charge) {
              ...
              // update project
              Projects.update({_id: project._id}, { $set: { soldGifts: soldGifts, gifts: _gifts, funded: newTotal }});
              // create offer notice
              Offers.insert(options);
              // create receipt
    ...
    */
    ...
    offerProjectAsset: function(options) {
      ...
      if (Meteor.user().notification_preferences&&Meteor.user().notification_preferences.email&&Meteor.user().notification_preferences.email.verification) {
        o.offer.user.email = Meteor.user().notification_preferences.email.em
      };

      if (o.expressOffer.days) {
        var d = new Date()
        d.setHours(d.getHours() + (o.expressOffer.days * 24))
        o.expiration = d
      };

      Offers.insert(o)

      if (me.notification_preferences&&me.notification_preferences.email&&me.notification_preferences.email.verification)
        o.offerorEmail = me.notification_preferences.email.email
      else
