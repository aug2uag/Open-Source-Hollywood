...
Meteor.methods({
  ...
  acceptUserToProject: function(options) {
    ...
    if (Meteor.isServer) {
      ...
      function finalUpdateHandler(idx) {
        ...
        var textMessage = 'Congratulations! You were selected and approved to join the campaign titled: ' + project.title + '. You now have access to the project board where tasks will be assigned to you. To access the board select BOARDS from the menu.';
        notifyByEmailAndPhone({
          user: options.user.id,
          message: textMessage,
          title: 'DECISION FOR THIS CAMPAIGN',
          subject: 'Application Decision from O . S . H .'
        });
      }
  ...
  rejectUserFromProject: function(offers) {
    ...
    if (Meteor.isServer) {
      ...
      var refundAmount = 0;
      offers.forEach(function(options, idx) {
        var key=options.ctx==='crew'?'crewApplicants':'roleApplicants';
        var applicants = project[key];
        for (var i = 0; i < applicants.length; i++) {
          var el = applicants[i];
          if (options.ctx===el.ctx&&options.type===el.type&&options.pay===el.pay&&options.equity===el.equity&&options.appliedFor===el.appliedFor&&options.created.toString()===el.created.toString()) {
            if (options.receipt&&el.receipt) {
              if (options.receipt.id===el.receipt.id) {
                message += 'Your application for '+options.appliedFor+' was declined and you were refunded $'+options.pay+'. ';
                if (idx===offers.length) return finalUpdateHandler(i);
              };
            };
          };
        };
