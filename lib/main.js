...
function declinedUserRefund(tx, project, amount, receipts, cb, user) {
    var stripe = require("stripe")(
        StripeServerKey
    );
...
Meteor.methods({
    ...
    addProject: function (options, cb) {
        ...
        request( requestParams, Meteor.bindEnvironment(function ( err, response, geoOptions ) {
          ...
          if (validOptions) {
            ...
            if (!project.needs||project.needs.length||!project.cast||!project.cast.length||!project.crew||!project.crew.length) {
              msg += 'Your new project can add or change cast / crew / resource requests in EDIT PROJECT. '
              if (!project.gifts||!project.gifts.length) {
                msg += 'Merch can be added there also.'
              };
            } else {
              if (!project.gifts||!project.gifts.length) {
                msg += 'Merch for your new campaign can be added in EDIT PROJECT.'
              };
            }

            if (!msg) {
              msg = 'Project created! Check your Dashboard for information about applicants, and to share your project with the world.'
            };
    ...
    */
    ...
    revokeOffer: function(options) {
      // console.log(options)
      check(options, Array)
      if (Meteor.isClient) return;


      // console.log(new Array(100).join('o '))
      // console.log(options)
    ...
    userConfig: function(options) {
      check(options, Object)
      var u = {}

      if (!myEmail()&&!options.email) {
        if (Meteor.isClient) alert('Include your email as part of your agreement to the terms of services.')
        return true
      } else {
        
        if (!myEmail()) {
          
          var email = options.email&&options.email.indexOf('@') > -1 ? options.email.trim() : null
          if (!email) {
            if (Meteor.isClient) alert('Include your email as part of your agreement to the terms of services.')
            return true
          } else {
            ...
            u.notification_preferences.email = {};
            u.notification_preferences.email.email = email
            u.notification_preferences.email.verification = true // make false; for production
            
            u.guid1 = guid();
            u.guid2 = guid();

            var verificationURL = BASE_URL + 'verify/' + u.guid2 + '/' + u.guid1;
            var _html = emailToHTML('VERIFY EMAIL', 'Your email was provided as a source for notifications on O . S . H .. To verify, please select the following:', verificationURL, 'SELECT TO VERIFY');
            var _text = "Your email was provided as a source for notifications on O . S . H .. To verify, please visit the following URL: " + verificationURL;
            var _subject = "Verify Email for Notifications from O . S . H .";
            sendEmailNotification(email, _html, _text, _subject);
    ...
    userViewAgreement: function(options) {
      check(options, Object)
      // console.log('userConfig')
      if (Meteor.isClient) return;
      Users.update({_id: Meteor.user()._id}, {$set: { userViewAgreement: true}})
      return
    },
    projectViewAgreement: function(options) {
      check(options, Object)
      // console.log('userConfig')
      if (Meteor.isClient) return;
      Users.update({_id: Meteor.user()._id}, {$set: { projectViewAgreement: true}})
      return
    },
    homeRouter: function() {
      return Meteor.user().iamRoles
    },
    userBoardDidOnboard: function() {
      if (Meteor.isClient) return;
      Users.update({_id: Meteor.user()._id}, {$set: { onboardBoarding: true}})
      return
    },
    addPersonalMerch: function(gift) {
      check(gift, Object)
      if (Meteor.isClient) return;
      if (gift.data) {
        var fn = guid() + new Date().getTime();
